...
#. Copy the parselyandroid directory to your project's top-level package directory (in a default Android Studio project, this is ``/app/src/main/java/com``.) The directory tree should look like ``/app/src/main/java/com/parsely/parselyandroid``.
#. In ``Build -> Edit Libraries and Dependencies`` under the ``Dependencies`` tab, use the green + to add two Library Dependencies: ``org.codehaus.jackson:jackson-core-lgpl:1.9.13`` and ``org.codehaus.jackson:jackson-mapper-lgpl:1.9.13``
...
#. Add the following lines to ``app/build.gradle:``
